...
_XML_ stands for Extensible Markup Language and it was designed as a structured way to share data. Because the data exist in a software/ hardware indepdent format this format allows for exchange between users.[^1] It it because of this platform indepdence and human and machine readable nature that XML documents are often used as the payload for API calls.[^2] This means that a user can request data via an API and an XML will be supplied in return.
...
There are two major packages that are useful for parsing XML type documents. There is the older `XML` package and the newer (circa 2015) `xml2`. Both packages have similar functions and capabilties for parsing XML type documents.  `XML` is the more comphrensive of the two packages, but it has a higher learning curve. Additionally, while piping and tidy principles can be applied using the `XML` package, the `xml2` package was designed by Jim Hester of RStudio with the _tidy_ programming paradigm in mind. While the package isn't quite as comphrensive (fewer functions) most of the main operations can be completed and the syntactic "tidyverse" sugar makes it easy to accomplish familiar tasks (e.g. `read_xml`).
